...
namespace NFSe
{
    class SP
    {
        ...
        /// <summary>
        /// Método responsável por efetuar a consulta de notas ficais no periodo selecionado.
        /// </summary>
        /// <param name="pathXml">Diretório onde serão salvos os arquivos +  nome do arquivo a ser gerado.</param>
        /// <param name="cnpj">CNPJ do Prestador.</param>
        /// <param name="numeroLote">Número do lote a ser consultado.</param>
        /// <returns>Retorna o nome do arquivo gerado.</returns>
        public static string ConsultarLotePeriodo(string pathXml, string cnpjEmitente, string cnpjconsulta, string dtInicio, string dtFim, string numeroPagina, string inscricao)
        {
            try
            {
                if (System.IO.File.Exists(pathXml))
                ...
                xml.WriteStartElement("PedidoConsultaNFePeriodo");
                xml.WriteAttributeString("xmlns", "http://www.prefeitura.sp.gov.br/nfe");
                xml.WriteStartElement("Cabecalho");
                xml.WriteAttributeString("Versao", "1");
                xml.WriteAttributeString("xmlns", "");
                xml.WriteStartElement("CPFCNPJRemetente");
                xml.WriteElementString("CNPJ", cnpjEmitente);
                xml.WriteEndElement(); //fim elemento CPFCNPJRemetente
                xml.WriteStartElement("CPFCNPJ");
                xml.WriteElementString("CNPJ", cnpjconsulta);
                xml.WriteEndElement(); //fim elemento Cabecalho
                xml.WriteElementString("Inscricao", inscricao);
                //xml.WriteElementString("Inscricao", inscricao.GetSubstring(0, 8).PadLeft(8, '0'));
                xml.WriteElementString("dtInicio", dtInicio);
                xml.WriteElementString("dtFim", dtFim);
                xml.WriteElementString("NumeroPagina", numeroPagina);
        ...
        /// <param name="numeroNfse">Número da Nfs-e.</param>
        /// <returns>Retorna o nome do arquivo gerado.</returns>
        public static string ConsultarCNPJ(string pathXml, string cnpj)
        {
            try
            {
                if (System.IO.File.Exists(pathXml))
                ...
                xml.WriteStartElement("PedidoConsultaCNPJ");
                xml.WriteAttributeString("xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance");
                xml.WriteAttributeString("xmlns:xsd", "http://www.w3.org/2001/XMLSchema");
                xml.WriteAttributeString("xmlns", "http://www.prefeitura.sp.gov.br/nfe");
                xml.WriteStartElement("Cabecalho");
                ...
                xml.WriteElementString("CNPJ", cnpj);
                xml.WriteEndElement(); //fim elemento CNPJContribuinte
                xml.WriteEndElement(); //fim elemento PedidoConsultaNFe
                xml.Close();
        ...
        /// 
        public static List<RespostaEnvioConsulta> TransmitirServicos(X509Certificate2 certificado, string pathXml, string metodo, int numLote, string numNfse = "")
        {
            ...
            try
            {
                ...
                switch (metodo)
                {
                    ...
                    case "consultaCNPJ":
                        ...
                        break;
                    case "consultaNfRecebidas":
                        respostaWebService = ws.ConsultaNFeRecebidas(1, xmlAssinado.OuterXml);
                        break;
                    case "consultaNfEmitidas":
                        respostaWebService = ws.ConsultaNFeEmitidas(1, xmlAssinado.OuterXml);
                        break;
                ...
                string arquivoResposta = @"C:\#PROJETOS\#ContadorVirtual\#Altevir\NFSe\Testes\Respostas\Resp_" + System.IO.Path.GetFileName(pathXml);
