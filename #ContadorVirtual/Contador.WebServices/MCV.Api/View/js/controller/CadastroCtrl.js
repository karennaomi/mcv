    var app = angular.module("cadastroMCV", ["ngMessages"]).config(function ($httpProvider) {
     ...
     angular.module("cadastroMCV").controller("cadastroMCVCtrl", function ($scope, $http) {
      ...
      $scope.adicionarUser = function (usuario, index) {
          $http.post("http://localhost:61406/api/users", usuario).success(function(data) {
      ...
      $scope.consultarCNPJ = function (index) {
          $http.post("http://localhost:61406/api/empresas/consultaCNPJ", usuario).success(function (data) {

              $scope.showMe = cnpjnok;

          }).error(function() {
              $scope.showMe = cnpjnook;
          });
