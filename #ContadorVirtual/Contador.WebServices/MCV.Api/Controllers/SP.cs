    ...
    public class SP
    {
        ...
        public static string GerarXml(string pathXml, Nota nfse, X509Certificate2 certificado)
        {
            ...
            try
            {
                ...
                xml.WriteElementString("dtInicio", nota.dataEmissao.Substring(0, 10));
                xml.WriteElementString("dtFim", nota.dataEmissao.Substring(0, 10));
                ...
                xml.WriteElementString("InscricaoPrestador", nota.IMPrestador.Substring(0, 8)); //Inscricao Municipal Prestador
                ...
                xml.WriteElementString("DataEmissao", nota.dataEmissao.Substring(0, 10)); //data emissao do rps (AAAA-MM-DD)
                ...
                xml.WriteElementString("RazaoSocialTomador", nota.razaoSocialTomador.Substring(0, 75));
                ...
                xml.WriteElementString("Logradouro", nota.endereco.Substring(0, 50));
                xml.WriteElementString("NumeroEndereco", nota.numeroEndereco.Substring(0, 10));

                if (!string.IsNullOrEmpty(nota.complemento))
                {
                    xml.WriteElementString("ComplementoEndereco", nota.complemento.Substring(0, 30));
                }

                xml.WriteElementString("Bairro", nota.bairro.Substring(0, 30));
                ...
                xml.WriteElementString("EmailTomador", nota.emailTomador.Substring(0, 60));
                xml.WriteElementString("Discriminacao", nota.discriminacaoServico.Substring(0, 2000));
        ...
        public static string ConsultarNfse(string pathXml, string cnpj, string inscricaoMunicipal, string numeroRps, string serieRps)
        {
            try
            {
                ...
                xml.WriteAttributeString("xmlns", "");
                xml.WriteStartElement("ChaveRPS");
                xml.WriteElementString("InscricaoPrestador", inscricaoMunicipal.Substring(0, 8).PadLeft(8, '0'));
        ...
        public static string CancelarNfse(string pathXml, string cnpj, string inscMunicipal, string numroNfse, X509Certificate2 certificado)
        {
            try
            {
                ...
                xml.WriteElementString("InscricaoPrestador", inscMunicipal.Substring(0, 8).PadLeft(8, '0'));
                xml.WriteElementString("NumeroNFe", numroNfse);
                xml.WriteEndElement(); //fim elementro ChaveNFe
                xml.WriteElementString("AssinaturaCancelamento", CertificadoDigital.SignAssinaturaHash(certificado, inscMunicipal.Substring(0, 8).PadLeft(8, '0') + numroNfse.PadLeft(12, '0')));
                xml.WriteEndElement(); //fim elemento Detalhe
                xml.Close();
        ...
        public static List<RespostaEnvioConsulta> ProcessarRespostas(string arquivoEnvio, string arquivoResposta, string metodo, int numLoteGerado, string numNfse)
        {
            ...
            if (ds.Tables.Contains("Cabecalho") && ds.Tables.Contains("InformacoesLote"))
            {
                ...
                resposta.dataRecebimento = Funcoes.RetornarCampoXml(ds, "InformacoesLote", 0, "DataEnvioLote", false).Replace("T", " ").Substring(0, 19);
                        ...
                        if (listaAlertas != null)
                        {
                            if (listaAlertas.Count > 0)
                            {
                                for (int b = 0; b < listaAlertas.Count; b++)
                                {
                                    ...
                                    if (Convert.ToDecimal(listaAlertas[b].Substring(6, 10)) == Convert.ToDecimal(resposta.numeroRps))
                                    {
