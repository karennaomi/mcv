...
union 
select log_localidade.ufe_sg, log_localidade.loc_no,'' as bai_no,'' as log_no, log_localidade.cep, '' as log_complemento,'' as nome
from log_localidade
...
delete from cep_todos where nome = 'UD Barra do Garças'

select cep, count(cep) from cep_todos group by cep having count(cep) > 1

ALTER TABLE cep_todos ADD CONSTRAINT PK_CEP PRIMARY KEY CLUSTERED (cep);
